...
public class OTBetterInstr extends EAInst {
	...
	public static void main(String args[]){
		...
		OTBetterInstr dvInst = new OTBetterInstr();
...
//		else 
	...
	@Override public void run() {
		...
		//System.out.println("myMethods="+myMethods);
		if (myMethods!=null && myMethods.size()>1) {
			instrument(myMethods);
		}
		else  {
			instrument();
		}	

		this.instRetEvents(myMethods);
	    this.instCommEvents(myMethods);
	}
	...
	public void instRetEvents(HashSet<String> methodArray) {
		...
		while (clsIt.hasNext()) {
			...
			while (meIt.hasNext()) {
				...
				if (methodArray!=null && methodArray.size()>1) 
	...
	public void instCommEvents(HashSet<String> methodArray) {
		/* traverse all classes */
		Iterator<SootClass> clsIt = Scene.v().getClasses().iterator();
		while (clsIt.hasNext()) {
			SootClass sClass = (SootClass) clsIt.next();
