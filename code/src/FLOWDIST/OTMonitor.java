...
//		A_distEA.clear();
		g_lgclock.initClock(1);
		//System.out.println("**************OTMonitor2::initialize()  2th");
		...
		Runtime.getRuntime().addShutdownHook( new Thread()
        {
          public void run()
          {
    		...
    		try {
    			bInitialized = true;
				OTMonitor.terminate("Forced upon external termination");
			} catch (Exception e) {
				e.printStackTrace();
			}
          }
...
//		System.out.println("debugOut="+debugOut);
		System.out.println("OTMonitor2 starts working ......");
		trackingSender=true;
		...
		if (trackingSender) {
			...
			//System.out.println("OTMonitor S.put(getProcessID(), Integer.MAX_VALUE) getProcessID()="+getProcessID());
	...
	protected synchronized static void serializeEvents() {
		...
		if ( !fnEventMaps.isEmpty() ) {
			...
			try {
				...
				// First we need the method index for indexing methods because the full sequence does not contain method name
				System.out.println("serializeEvents() A.size()="+A.size());
				oos.writeObject(S);
				oos.writeObject(A);
				oos.flush();
				oos.close();
