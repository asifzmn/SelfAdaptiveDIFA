...
public class dtThreadMonitor {
	...
	public synchronized static void initialize() throws Exception{
		System.out.println("**************dtThreadMonitor::initialize()  0th");
		getCreateF().clear();
		getCreateL().clear();
		getCreateA().clear();
		getCreateS().clear();
		...
		synchronized (g_counter) {
			...
			System.out.println("**************dtThreadMonitor::initialize()  1th");
			if (bFullSequence) {
		...
		System.out.println("**************dtThreadMonitor::initialize()  2th");
		...
		Runtime.getRuntime().addShutdownHook( new Thread()
        {
          public void run()
          {
    		...
    		try {
    			bInitialized = true;
				dtThreadMonitor.terminate("Forced upon external termination");
			} catch (Exception e) {
				e.printStackTrace();
			}
          }
